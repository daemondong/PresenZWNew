...
extern XBYTE supervision_session_id;
#else
BYTE supervision_session_id = 0;
#endif
  ...
  BYTE cmdLength)               /* IN Number of command bytes including the command */
{
  ...
  switch (pCmd->ZW_Common.cmd)
  {
    ...
    case SUPERVISION_REPORT:
      if ((supervision_session_id - 1) == pCmd->ZW_SupervisionReportFrame.properties1)
      {
...
void CommandClassSupervisionGetAdd(ZW_SUPERVISION_GET_FRAME* pbuf)
{
  ...
  supervision_session_id = ((supervision_session_id + 1) > 0x3F) ? 1 : (supervision_session_id + 1); /* increment m_sessionId, wrap around if over 0x3F */
  ZW_DEBUG_SEND_BYTE('e');
  ZW_DEBUG_SEND_NUM(supervision_session_id);
}

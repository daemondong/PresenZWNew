  ...
  BYTE cmdLength)               /* IN Number of command bytes including the command */
{
  ...
  switch (pCmd->ZW_Common.cmd)
  {
    ...
    case SWITCH_ALL_SET:
      if(NUMBER_OF_ENDPOINTS >= rxOpt->destNode.endpoint)
      {
        MemoryPutByte(  (WORD)&EEOFFSET_SWITCH_ALL_MODE_far[rxOpt->destNode.endpoint] ,pCmd->ZW_SwitchAllSetFrame.mode);
        return RECEIVED_FRAME_STATUS_SUCCESS;
      }
      return RECEIVED_FRAME_STATUS_FAIL;
      break;
    ...
    case SWITCH_ALL_OFF:
      if(NUMBER_OF_ENDPOINTS >= rxOpt->destNode.endpoint)
      {
        switchAllMode =  MemoryGetByte( (WORD)&EEOFFSET_SWITCH_ALL_MODE_far[rxOpt->destNode.endpoint]);
        if (switchAllMode & SWITCH_ALL_REPORT_EXCLUDED_FROM_THE_ALL_ON_FUNCTIONALITY_BUT_NOT_ALL_OFF)
        {
          handleSwitchAll(CMD_CLASS_SWITCHALL_OFF, rxOpt->destNode.endpoint);
        }
        return RECEIVED_FRAME_STATUS_SUCCESS;
      }
      return RECEIVED_FRAME_STATUS_FAIL;
    ...
    case SWITCH_ALL_ON:
      if(NUMBER_OF_ENDPOINTS >= rxOpt->destNode.endpoint)
      {
        ...
        {
          handleSwitchAll(CMD_CLASS_SWITCHALL_ON, rxOpt->destNode.endpoint);
        }
        return RECEIVED_FRAME_STATUS_SUCCESS;
      }
      return RECEIVED_FRAME_STATUS_FAIL;

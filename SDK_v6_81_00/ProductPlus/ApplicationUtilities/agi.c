                       ...
                       uint8_t endpoint)
{
  ...
  {
    memcpy((uint8_t *)myAgi.lifeLineEndpoint[endpoint].grpName, (uint8_t * )pGrpName, AGI_STRING_LEN - 1);
    myAgi.lifeLineEndpoint[endpoint].grpName[AGI_STRING_LEN - 1] = '\0'; /* secure null character */
  }
  else
  {
    memcpy((uint8_t *)myAgi.lifeLineEndpoint[endpoint].grpName, (uint8_t * ) pGrpName, stringLen);
  }
}

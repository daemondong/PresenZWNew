 ...
 */
void
ZAF_eventSchedulerInit(VOID_CALLBACKFUNC(pApplicationStateMachine)(BYTE))
{
...
PCB_BOOL(ZCB_eventScheduler)(void)
{
  ...
  return ZW_util_queue_GetItemCount(&eventQueueEngine) != 0;
}

BOOL
ZAF_jobEnqueue(BYTE event)
{
...
uint8_t
ZAF_jobQueueCount(void)
{
  return ZW_util_queue_GetItemCount(&jobQueueEngine);
}

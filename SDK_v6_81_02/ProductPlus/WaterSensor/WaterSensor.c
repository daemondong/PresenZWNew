...
static BYTE supportedEvents[TOTALEVENTS] = { 0,0x02 };
...
//								ZCB_EventEnqueue(EVENT_APP_NOTIFICATION_START_JOB);
							...
							if (Key5S==1 && KeyPress>0) {
								KeyPress++;
								Handle_ONEPress=TimerStart(ZCB_ONEPress, 30, 1);
								if (KeyPress==6) {
									ledFlickTimes=3;
									KeyPress=0;
//									ZCB_SetPowerDownTimeout(LEARNMODE_POWERDOWNTIMEOUT);
					...
					case MYKEY02:
						...
						if ((nowval & (1<<i))==0) { basicValue = 0x0; myEvent = 0; }     //按下为零
						else { basicValue = 0xFF; myEvent = 1; }
...
ApplicationInitSW(ZW_NVM_STATUS nvmStatus)
{
  ...
  initMyUtil();
  if (LedLight==0 && (wakeupReason == ZW_WAKEUP_RESET || wakeupReason == ZW_WAKEUP_POR)) {
	ledOnTimes = LED_ON_TIME;
	LedLight=1;
...
//      if((EVENT_KEY_B1_DOWN == event)||(EVENT_SYSTEM_LEARNMODE_END == event))
	  if(EVENT_SYSTEM_LEARNMODE_END == event)
      {
        ZW_NetworkLearnModeStart(E_NETWORK_LEARN_MODE_DISABLE);
        ZCB_eventSchedulerEventAdd(EVENT_APP_INIT);
        ChangeState(STATE_APP_STARTUP);

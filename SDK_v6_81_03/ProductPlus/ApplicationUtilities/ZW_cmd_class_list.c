                  ...
                  BYTE nonSecurelistLen)
{
  ...
  {
    if ((COMMAND_CLASS_BASIC == cmdClass) ||
        (TRUE == CheckCmdClass(cmdClass, pSecurelist, securelistLen)))
    {
      return TRUE; /*cmd is supported!*/
    }
  }

  if ((SECURITY_KEY_NONE != device_higest_secure_level) &&   /* Securely included */
      (SECURITY_KEY_NONE == eKey) &&                         /* non-secure input */
      (SECURITY_KEY_S0 == device_higest_secure_level) &&     /* Security S0 */

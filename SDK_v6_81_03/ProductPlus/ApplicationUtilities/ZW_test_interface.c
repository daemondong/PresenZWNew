...
#define ZDB_SEND_NUM ZW_UART0_tx_send_num
...
void init_once(char mode, unsigned char len);
...
void init_once(char mode, unsigned char len)
{
  ...
  {
    ZW_test_interface_driver_init(mode,len);
...
BOOL ZW_test_interface_allocate(char channel, VOID_CALLBACKFUNC(pCallback) (char channel, char * pString), char mode, unsigned char len)
{
...
PCB_BOOL(ZCB_test_interface_poll)(void)
{
  ...
  if (dataLength==2)
  {
    ...
    {
        get_index_from_channel(&channel_index, 'c');
		buffer[2]=0;
		(channels[channel_index].pCallback)('c', (char *)&buffer[0]);
        channel_status = TRUE;
    }
    if(FALSE == channel_status)
    {

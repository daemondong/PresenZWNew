...
void ZW_test_interface_driver_init(char mode, unsigned char len)
{
  ZDB_UART_INIT(96, TRUE, TRUE);
  ...
  ZDB_UART_DMA_INT_BYTE_COUNT(len);
  if (mode==0) {
	...
	ZDB_UART_DMA_INIT(dmaBuffer, DMA_BUFFER_SIZE_SINGLE, (UART_RX_DMA_SWITCH_EOR | UART_RX_DMA_SWITCH_COUNT | UART_RX_DMA_SWITCH_FULL));
	ZDB_UART_DMA_EOR_SET(DMA_EOR_CHARACTER);
	ZW_UART0_rx_dma_byte_count_enable(TRUE);
  }
}

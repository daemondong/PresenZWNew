                                       ...
                                       uint8_t sourceEndpoint)
{
  ...
  {
    uint8_t i;
    ZW_DEBUG_AGI_SEND_STR("Node list: ");
    for(i = 0; i < pTxOptionsEx->list_length; i++)
    {
      ZW_DEBUG_AGI_SEND_NUM(pTxOptionsEx->pList[i].node.nodeId);
      ZW_DEBUG_AGI_SEND_BYTE('.');
      ZW_DEBUG_AGI_SEND_NUM(pTxOptionsEx->pList[i].node.endpoint);
      ZW_DEBUG_AGI_SEND_BYTE(';');
    }
  ...
  {
    ZW_DEBUG_AGI_SEND_NL();
    ZW_DEBUG_AGI_SEND_STR("->Node list error #:");
    ZW_DEBUG_AGI_SEND_NUM(status);
    ZW_DEBUG_AGI_SEND_NL();
  }

                        ...
                        uint8_t                   cmd_length)
{
  ...
  if (set_cmd->value < 0x64 || 0xFF == set_cmd->value)
  {
    ...
    if (false == ZAF_TSE_Trigger((void *)CC_Indicator_report_stx, &indicatorData, true))
    {
                        ...
                        uint8_t                   cmd_length)
{
  ...
  const int frame_size_fixed_part = offsetof(ZW_INDICATOR_SET_1BYTE_V3_FRAME,
                                             variantgroup1);
                        ...
                        uint8_t                   cmd_length)
{
  ...
  EQueueNotifyingStatus            queue_status = EQUEUENOTIFYING_STATUS_SUCCESS;
  ...
  if (EQUEUENOTIFYING_STATUS_SUCCESS != queue_status)
  {
                                 ...
                                 uint8_t                   cmd_length)
{
  ...
  EQueueNotifyingStatus                         queue_status = EQUEUENOTIFYING_STATUS_SUCCESS;
  uint8_t                                       send_data_length = 0;

  RxToTxOptions(rx_opt, &tx_opt);

  if (INDICATOR_IND_NODE_IDENTIFY == sget_cmd->indicatorId)
  {
  ...
  else
  {
    ...
    if (INDICATOR_IND_NA == sget_cmd->indicatorId)
    {
      /*
       * If indicator ID == INDICATOR_IND_NA (0x00) the client has
       * requested to discover supported indicator IDs. We advertise
       * the first (and only) indicator ID by setting nextIndicatorId
       * in the otherwise empty report.
       */
//      report->nextIndicatorId = INDICATOR_IND_NODE_IDENTIFY;
        report->indicatorId = INDICATOR_IND_NODE_IDENTIFY;
    }
...
handleCommandClassIndicator(RECEIVE_OPTIONS_TYPE_EX  *rx_opt,
                            ZW_APPLICATION_TX_BUFFER *cmd,
                            uint8_t                   cmd_length)
{
...
void CC_Indicator_report_stx(TRANSMIT_OPTIONS_TYPE_SINGLE_EX txOptions, s_CC_indicator_data_t* pData)
{
  DPRINTF("* %s() *\n"
      "\ttxOpt.src = %d\n"
      "\ttxOpt.options %#02x\n"
  ...
  prepare_report_v3(pData->indicatorId, pTxBuf, &payloadLen);
...
prepare_report_v3(uint8_t indicatorId, ZW_APPLICATION_TX_BUFFER *pTxBuffer, uint8_t *payloadLen)
{
  if(INDICATOR_IND_NODE_IDENTIFY == indicatorId)
  {
    ...
    pTxBuffer->ZW_IndicatorReport3byteV2Frame.variantgroup3.value       = g_on_time;
...
REGISTER_CC(COMMAND_CLASS_INDICATOR, INDICATOR_VERSION_V3, handleCommandClassIndicator);

...
void DefaultNotificationStatus(NOTIFICATION_STATUS status)
{
  ...
  memset(&tDefaultAlarmStatus, 0, sizeof(tDefaultAlarmStatus));
  ...
  if( true == FindNotificationEndpoint(notificationType, &endpoint) )
  {
    ...
    *pNbrBitMask = 0;
    ...
    for( i = 0; i < myNotification.grp[grpNo].supportedEventsLen; i++)
    {
      ...
      pBitMaskArray[myNotification.grp[grpNo].pSupportedEvents[i]/8] |= (0x01<< (myNotification.grp[grpNo].pSupportedEvents[i]%8));
    }
    ...
    uint8_t endpoint)
{
  ...
  uint8_t tmpEvent= *pNotificationEvent;
  *pEventPar = 0;
  *pEvNbrs = 0;
  if(0xff == grpNo)
  {
  ...
  if(true == ValidateNotificationType(*pNotificationType, endpoint ))
  {
    ...
    for(i = 0; i < myNotification.grp[grpNo].supportedEventsLen; i++)
    {
      ...
      if((*pNotificationEvent == myNotification.grp[grpNo].pSupportedEvents[i]) && (0x00 != *pNotificationEvent))
      {
        ...
        {
          /*No event in queue*/
          if (*pNotificationEvent == 0x00 && tmpEvent == 0xFE) *pNotificationEvent = 0xFE;
          *pEventPar = 0;
          return true;
        }
      }

...
typedef enum _EVENT_APP_
{
  ...
  EVENT_APP_UART1_DATA,
  EVENT_APP_UART1_DATA_START,
  EVENT_APP_UART1_DATA_STOP,
  EVENT_APP_UART1_DATA_INCLUDE,
  EVENT_APP_UART1_DATA_RESET
}
EVENT_APP;
...
static uint8_t supportedEvents[EVENTS_NUMBER] = {NOTIFICATION_EVENT_SMOKE_ALARM_SMOKE_DETECTED,NOTIFICATION_EVENT_SMOKE_ALARM_TEST};
...
volatile struct circularBuffer
{
  ...
  bool     readOver;          /* buffer overflow indicator */
} rxBuf, txBuf = { {0}, 0, 0, 0, false, false };

/* Setup UART1 in async mode for RS232*/
//static USART_TypeDef           * uart   = USART0;


//Application specific files should preferably use identifiers in the range 0x00000¡ª0x0FFFF
#define ZAF_FILE_ID_MY_DATA      0
...
//  ZAF_UART0_enable(115200, true, true);
  ...
  ZAF_UART1_enable(9600, true, true); // Enable USART1 (for Rx only)
...
AppStateManager(EVENT_APP event)
{
  ...
  if (EVENT_APP_UART1_DATA == event) {
	  uint8_t i,v;
	  v=0;
	  for (i=4;i<rxBuf.wrI-1;i++) v ^= rxBuf.data[i];
	  if (v == rxBuf.data[i]) {
		  ...
		  case 0xA1:
			  v=0;
			  if ((rxBuf.data[7] & 0x20)>0) v=2;
			  if ((rxBuf.data[6] & 0x80)>0) v=1;
			  if (v>0) {
    			  My_Data.LastEventID=v;
    			  nvm3_writeData(pFileSystemApplication, ZAF_FILE_ID_MY_DATA, &My_Data, sizeof(struct MY_DATA));
    			  ZAF_EventHelperEventEnqueueFromISR(EVENT_APP_UART1_DATA_START);
			  }
			  else ZAF_EventHelperEventEnqueueFromISR(EVENT_APP_UART1_DATA_STOP);
		  break;

		  case 0xA5:
			  ZAF_EventHelperEventEnqueueFromISR(EVENT_APP_UART1_DATA_INCLUDE);
		  break;

		  case 0xA6:
			  ZAF_EventHelperEventEnqueueFromISR(EVENT_APP_UART1_DATA_RESET);
		  break;
		  }
	  }
	  else DPRINT("\r\n*!*!****************** UART1_DATA_ERROR");
	  rxBuf.wrI=0;
	  rxBuf.readOver=false;
  }

  if ((BTN_EVENT_LONG_PRESS(APP_BUTTON_LEARN_RESET) == (BUTTON_EVENT)event) || (EVENT_SYSTEM_RESET == (EVENT_SYSTEM)event) || EVENT_APP_UART1_DATA_RESET==event)
  {
...
//    ZAF_PM_Cancel(&m_RadioPowerLock);
  ...
  if (BTN_EVENT_SHORT_PRESS(PIR_EVENT_BTN) == (BUTTON_EVENT)event) {
	  ZAF_PM_StayAwake(&m_RadioPowerLock, 1000);
  }
...
*/        AddNotification(
    ...
    case STATE_APP_IDLE:
          ...
          (BTN_EVENT_HOLD(PIR_EVENT_BTN) == (BUTTON_EVENT)event))
      {
        ...
        /*Add event's on job-queue*/
        if (My_Data.LastEventID!=2) {
        	My_Data.LastEventID=2;
            nvm3_writeData(pFileSystemApplication, ZAF_FILE_ID_MY_DATA, &My_Data, sizeof(struct MY_DATA));
        }
      ...
      if (EVENT_APP_UART1_DATA_START <= event && EVENT_APP_UART1_DATA_RESET > event) {
		  DPRINT("\r\n*!*!****************** UART1_EVENT_BTN");
    	  switch (event) {
    	  case EVENT_APP_UART1_DATA_START:
    		  ZAF_PM_StayAwake(&m_RadioPowerLock, 0);
    		  ChangeState(STATE_APP_TRANSMIT_DATA);
    	  ...
    	  case EVENT_APP_UART1_DATA_STOP:
    		  ZAF_PM_StayAwake(&m_RadioPowerLock, 0);
    		  ChangeState(STATE_APP_TRANSMIT_DATA);
    	  ...
    	  case EVENT_APP_UART1_DATA_INCLUDE:
    		  if (EINCLUSIONSTATE_EXCLUDED != g_pAppHandles->pNetworkInfo->eInclusionState){
    			  DPRINT("\r\nLEARN_MODE_EXCLUSION");
    			  ZAF_setNetworkLearnMode(E_NETWORK_LEARN_MODE_EXCLUSION_NWE, g_eResetReason);
    		  }
...
*/
      ...
      if (EVENT_APP_NOTIFICATION_START_JOB == event && My_Data.LastEventID>0)
      {
        DPRINT("\r\nEVENT_APP_NOTIFICATION_START_JOB");
        NotificationEventTrigger(&lifelineProfile,
        						 NOTIFICATION_TYPE_SMOKE_ALARM,
                                 supportedEvents[My_Data.LastEventID-1],
                                 NULL, 0,
                                 ENDPOINT_ROOT);
      ...
      if (EVENT_APP_NOTIFICATION_STOP_JOB == event && My_Data.LastEventID>0)
      {
        DPRINT("\r\nEVENT_APP_NOTIFICATION_STOP_JOB");
        NotificationEventTrigger(&lifelineProfile,
        						 NOTIFICATION_TYPE_SMOKE_ALARM,
...
SetDefaultConfiguration(void)
{
  ...
  errCode = nvm3_writeData(pFileSystemApplication, ZAF_FILE_ID_MY_DATA, &My_Data, sizeof(struct MY_DATA));
  ASSERT(ECODE_NVM3_OK == errCode); //Assert has been kept for debugging , can be removed from production code if this error can only be caused by some internal flash HW failure
...
LoadConfiguration(void)
{
  ...
  if (EFSV_STATUS_SUCCESS == eFileSetVerifyStatus)
  {
    ...
    nvm3_readData(pFileSystemApplication, ZAF_FILE_ID_MY_DATA, &My_Data, sizeof(struct MY_DATA));
...
void USART1_RX_IRQHandler(void)
{
  ...
  if (USART1->STATUS & USART_STATUS_RXDATAV)
  {
    ...
    if (rxBuf.readOver==false) {
    	rxBuf.data[rxBuf.wrI] = rxData;
    	rxBuf.wrI             = (rxBuf.wrI + 1) % BUFFERSIZE;
    	j=rxBuf.wrI;
    	for (i=0;j<5 && i<rxBuf.wrI;i++) {
    		if (i==0 && rxBuf.data[0]!=0xCC) j=0;
    		if (i==1 && rxBuf.data[1]!=0x99) j=0;
    		if (i==2 && rxBuf.data[2]!=0x99) j=0;
    		if (i==3 && rxBuf.data[3]!=0xCC) j=0;
    	}
    	rxBuf.wrI=j;
    	rxBuf.pendingBytes=j;
        ...
        if (rxBuf.wrI>6 && rxBuf.data[4]==(rxBuf.wrI-6)) {
        	ZAF_EventHelperEventEnqueueFromISR(EVENT_APP_UART1_DATA);
        	rxBuf.readOver=true;
        	DPRINT("\r\n*!*!****************** UART1_INT");
        }
    }

...
{ .ObjectKey = FILE_ID_APPLICATIONDATA,       .iDataSize = FILE_SIZE_APPLICATIONDATA     },
  ...
  switch(currentState)
  {
    ...
    case STATE_APP_IDLE:
      ...
      if (BTN_EVENT_SHORT_PRESS(SWITCH_TOGGLE_BTN) == (BUTTON_EVENT)event)
      {
        ...
        ZAF_TSE_Trigger((void *)CC_BinarySwitch_report_stx, pData, true);
      }
...
SetDefaultConfiguration(void)
{
  ...
  tApplicationData.onOffState = onOffState;

  writeAppData(&tApplicationData);
...
LoadConfiguration(void)
{
  ...
  /* Check to see, if any valid configuration is stored in the file system */
  SFileSet FileSet = {
...
{
  /* Basic Set Command is mapped to Binary Switch Set */
